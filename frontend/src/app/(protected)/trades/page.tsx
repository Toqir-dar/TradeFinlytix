  ...
  return (
    <div suppressHydrationWarning style={{ fontFamily: "'DM Sans', 'Segoe UI', sans-serif", color: th.text }}>
      <style>{`
        ...
        .filter-btn { padding: 8px 18px; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; border: 1.5px solid ${th.border}; background: ${th.card}; color: ${th.subtext}; font-family: inherit; transition: all 0.2s; }
        ...
        .trade-row { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr 1fr 1fr; gap: 8px; padding: 14px 16px; border-bottom: 1px solid ${th.borderSubtle}; align-items: center; transition: background 0.15s; }
        .trade-row:hover { background: ${th.innerCard}; border-radius: 8px; }
              ...
              <div style={{ display: "flex", gap: 6 }}>
                {(["buy", "sell"] as const).map(s => (
                  <button key={s} onClick={() => setSide(s)} style={{
                    ...
                    borderRadius: 10,
                    background: side === s ? (s === "buy" ? "#DCFCE7" : "#FEE2E2") : th.sideBtnUnselBg,
                    color: side === s ? (s === "buy" ? "#15803D" : "#991B1B") : th.sideBtnUnselColor,
                    fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "inherit", textTransform: "uppercase"
                  }}>
                    {s}
                  </button>
                ))}
          ...
          <div key={s.label} className="stat-card" style={{ position: "relative", overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div>
                ...
                <p style={{ fontSize: 24, fontWeight: 800, color: (s as any).color ?? th.text }}>{s.value}</p>
      ...
      <div className="responsive-grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 20, marginBottom: 24 }}>
        <div className="section-card">
          <h3 style={{ fontWeight: 700, fontSize: 16, marginBottom: 4, color: th.text }}>Trade Activity</h3>
          <p style={{ fontSize: 12, color: th.muted, marginBottom: 16 }}>Buy vs Sell (7 days)</p>
        ...
        <div className="section-card">
          ...
          {filtered.slice(0, 5).map((t: any) => (
            <div key={t._id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${th.borderSubtle}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: th.text }}>{t.trade.symbol}</div>
                  <div style={{ fontSize: 11, color: th.muted }}>{new Date(t.timestamp).toLocaleTimeString("en-PK", { hour: "2-digit", minute: "2-digit" })}</div>
                </div>
              </div>
              ...
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: th.text }}>PKR {t.trade.price.toFixed(1)}</div>
                <div style={{ fontSize: 11, color: th.muted }}>Qty: {t.trade.quantity}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="section-card">
        ...
        <div className="table-scroll">
          <div className="table-min">
            <div className="trade-row" style={{ borderBottom: `2px solid ${th.borderSubtle}`, padding: "8px 16px" }}>
            ...
            {filtered.map((t: any) => {
              ...
              return (
                <div key={t._id} className="trade-row">
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 500, color: th.text }}>{new Date(t.timestamp).toLocaleDateString("en-PK", { day: "numeric", month: "short", year: "numeric" })}</div>
                    <div style={{ fontSize: 11, color: th.muted }}>{new Date(t.timestamp).toLocaleTimeString("en-PK", { hour: "2-digit", minute: "2-digit" })}</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <div style={{ width: 32, height: 32, background: th.symbolIconBg, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: th.symbolIconColor }}>
                      {t.trade.symbol.slice(0, 3)}
                    </div>
                    <span style={{ fontWeight: 700, fontSize: 14, color: th.text }}>{t.trade.symbol}</span>
                  </div>
                  <span>
                    <span className="chip" style={{ background: t.trade.side === "buy" ? "#DCFCE7" : "#FEE2E2", color: t.trade.side === "buy" ? "#15803D" : "#991B1B" }}>
                      {t.trade.side}
                  ...
                  <span style={{ fontSize: 14, color: th.subtext, fontWeight: 500 }}>{t.trade.quantity.toLocaleString()}</span>
                  <span style={{ fontSize: 14, color: th.subtext }}>PKR {t.trade.price.toFixed(2)}</span>
                  <span style={{ fontSize: 14, fontWeight: 700, color: th.text }}>PKR {total.toLocaleString(undefined, { maximumFractionDigits: 0 })}</span>
                </div>
              );
            })}
