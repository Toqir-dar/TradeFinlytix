...
>>>>>>> 2294587 (feat: add theme toggle component for dark and light mode)
...
export function ProtectedShell({ children }: { children: React.ReactNode }) {
  ...
  if (loading || !user) return (
    <div suppressHydrationWarning style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: mono ? "#0f172a" : "#F0FDF4", fontFamily: "'DM Sans', sans-serif" }}>
          ...
          <div className="shell-right-controls">
            ...
            <div style={{ position: "relative" }}>
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="badge-pulse" style={{ position: "absolute", top: -4, right: -4, background: "#DC2626", color: "white", borderRadius: "50%", width: 18, height: 18, fontSize: 10, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", border: `2px solid ${sh.btnBg}` }}>
              ...
              {showAlerts && (
                <div id="alerts-panel" className="alerts-panel" style={{ position: "absolute", right: 0, top: "calc(100% + 8px)", width: "min(360px, 90vw)", background: sh.alertDropBg, border: `1.5px solid ${sh.alertDropBorder}`, borderRadius: 16, boxShadow: "0 16px 40px rgba(0,0,0,0.18)", overflow: "hidden", zIndex: 100 }}>
                  <div style={{ padding: "14px 16px", borderBottom: `1px solid ${sh.alertDivider}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontWeight: 700, fontSize: 15, color: sh.alertText }}>Notifications</span>
                    <span style={{ fontSize: 12, color: sh.alertMuted }}>{unreadCount} unread</span>
                  </div>

                  {alerts.length === 0 ? (
                    <div style={{ padding: "32px 16px", textAlign: "center", color: sh.alertMuted }}>
                    ...
                    <div style={{ maxHeight: 320, overflowY: "auto" }}>
                      {alerts.map((alert: any, i: number) => {
                        ...
                        return (
                          <div key={alert._id ?? i} className="alert-item" onClick={() => markAlertRead(alert._id)}
                            title={alert.is_read ? "Already read" : "Mark as read"}
                            style={{ background: alert.is_read ? sh.alertDropBg : sh.alertHover }}>
